CPUERatio<- function(CPUEDat,LagLength,Weight,Form,Iterations,BootStrap)
{
  ...
  colnames(MCDetails)<- c('Iteration','Year','FishedCPUE','CPUEDensity')
  ...
  for (i in 1:Iterations)
  {
    ...
    for (y in 1:length(Years))
    {
      ...
      MCDetails$CPUEDensity[c]<- WeightedIn
  ...
  if (Iterations>1)
  {
    ...
    for (y in 1:length(Years))
    {
      ...
      Output[y,]<- I(data.frame(Years[y],'CPUERatio',SampleSize$SampleSize[y],TrueOutput$Value[y],LowerCI,UpperCI,SD,'Biomass CPUE Ratio',TrueOutput$Flag[y],stringsAsFactors=F))
      
    }
    
  }
  
  MCDetails$CPUEDensity[MCDetails$CPUEDensity==0]<- NA
  
  pdf(file=paste(FigureFolder,' CPUE Ratio NvK Boxplots.pdf',sep=''))
  boxplot((MCDetails$FishedCPUE/MCDetails$CPUEDensity)~MCDetails$Year,frame=F,xlab='Year',ylab='Fished CPUE/Unfished CPUE',notch=T,outline=F)
  dev.off()
  ...
  boxplot((MCDetails$CPUEDensity)~MCDetails$Year,frame=F,xlab='Year',ylab='CPUE',notch=T,outline=F)
  dev.off()
