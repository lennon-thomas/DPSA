...
# Run DPSA Modules --------------------------------------------------------
# Created by Dan Ovando, July 20 2015
# This is the generic wrapper for the DPSA modules
...
source(paste("Blue tang\\Montserrat2-LittleBay-Blue tang_ControlFile.R"))
...
# GFD<- FindFishbase(LifeHistory)

Fish$AgeMat50<- NA

Fish$AgeMatSource<- NA

# Fish$t0[is.na(GFD$t0)]<- 0
...
#           {
            ...
            Temp<- LBAR(SampleCheck$ParedData,LagLength=1,Weight=1,IncludeMPA=0,ReserveYr=NA,OutsideBoundYr=NA,Iterations=1000,
                        BootStrap=1,LifeError=0,Lc=NA)$Output		
...
#                                ReserveYr=NA,OutsideBoundYr=NA,ManualM=1,GroupMPA=0,Iterations=NumIterations,BootStrap=0,LifeError=0,HistInterval=1)
# #             
...
#         {
          ...
          FMSYtoMratio <- 1 #ratio of Fmsy to M
          SD.FMSYtoMratio<- 0.5
          Delta<- 0.5
          SD.Delta<- 0.1
          DeltaLowerBound<- 0.4
          ...
          BMSYtoB0LowerBound<- 0.3
          BMSYtoB0UpperBound<- 0.7
          CatchInterp<-1
          NIter<- 500	
#           
#           
...
#       
      ...
      save.image(file=paste(ResultFolder,AssessmentName,'_Settings3.RData',sep='')) #Save settings used to produce current results
      write.csv(file=paste(ResultFolder,AssessmentName,'_Results3.csv',sep=''),CurrentResults) #Save current results
#       
